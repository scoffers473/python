...
class TextAnalysisTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(self.filename,'w') as f:
            f.write("Now we are engaged in a freat civil war.\n"
                    "testing whether that nation\n"
                     "or nation so conceived and so dedicated\n"
                     "can endure.")
        
    def tearDown(self):
        """Fixture that deletes the files used by the test method."""
        try:
    ...
    def test_charcter_count(self):
        "Check that the chatacter count is correct."""
        self.assertEqual(analyze_text(self.filename)[1],120)

    def test_no_such_file(self):
        """Check that the propper exception is thrown for a mising file."""
